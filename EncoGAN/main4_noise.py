'''
Yihan Jiang's update on Milestone 2.
 Code is a little bit messy, need to refactor for later use. ETA 04/16
(1) Add Noise Layer to picture
(2) Add DTA encoder/decoder:
(3) Refactor the code to make it easy to modiy.
'''
from __future__ import print_function
# from get_args import get_args
from get_args_noise import get_args
import os
import csv
import random
import torch
...
fixed_u = torch.randint(0, 2, (args.batch_size, ud,im,im), dtype=torch.float, device=device)
    ...
    for epoch in range(args.end_epoch):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            if (iters % 500 == 0) or ((epoch == args.end_epoch - 1) and (i == len(dataloader) - 1)):
                with torch.no_grad():
                    v_noise = torch.randn(args.batch_size, nz, 1, 1, dtype=torch.float,device=device)
                    v_u = torch.randint(0, 2, (args.batch_size, ud, im, im), dtype=torch.float,device=device)
