...
class TurboAE_decoder2D(nn.Module):
    def __init__(self, args, p_array):
        ...
        for idx in range(args.num_iteration):
            self.dec1_cnns.append(SameShapeConv2d(num_layer=args.dec_num_layer, in_channels=args.img_channels - 1 + args.num_iter_ft,
                                                  out_channels= args.dec_num_unit, kernel_size = args.dec_kernel_size)
            )

            self.dec2_cnns.append(SameShapeConv2d(num_layer=args.dec_num_layer, in_channels=args.img_channels - 1 + args.num_iter_ft,
                                                  out_channels= args.dec_num_unit, kernel_size = args.dec_kernel_size)
            )
...
class CNN_decoder2D(nn.Module):
    def __init__(self, args, p_array):
        ...
        self.cnn   = SameShapeConv2d(num_layer=args.dec_num_layer, in_channels=args.img_channels,
                                     out_channels= args.dec_num_unit, kernel_size = args.dec_kernel_size)

        self.final = SameShapeConv2d(num_layer=1, in_channels=args.dec_num_unit,
