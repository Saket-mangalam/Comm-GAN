...
class Hidden_Generator_7(nn.Module):
    def __init__(self, args):
        ...
        def up_block(in_feat, out_feat, normalize=True):
            block = [nn.ConvTranspose2d(in_feat,out_feat,4,stride=2,padding=1,bias=False)]
            if normalize:
                block.append(nn.BatchNorm2d(out_feat, 0.8))
            block.append(nn.LeakyReLU(0.2, inplace=True))
            return block
...
class Gan_Generator(nn.Module):
    def __init__(self, args):
        ...
        def conv_block(in_feat, out_feat, normalize=True, kernel = 4, pad = 2):
            layers = [nn.ConvTranspose2d(in_feat,out_feat,kernel,stride=2, padding=1,bias=False)]
            if normalize:
                layers.append(nn.BatchNorm2d(out_feat, 0.8))
            layers.append(nn.LeakyReLU(0.2, inplace=True))
            return layers
