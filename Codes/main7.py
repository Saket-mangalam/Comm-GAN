...
#size of bits per pixel latent vector
nu = 1
...
class Generator(nn.Module):
    def __init__(self, ngpu):
        ...
        self.encode = nn.Sequential(
            ...
            nn.Conv2d((nc + ngf + nc*nu), ngf, 3, 1, 1, bias=False),
            nn.BatchNorm2d(ngf),
            nn.LeakyReLU(0.2, inplace=True),
            # statesize is 64x64x64
    ...
    def forward(self, z, u):
        ...
        input = torch.cat([enc_z, u, real], dim=1)
...
class Decoder(nn.Module):
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(ndf * 4, nc*nu, 3, 1, 1, bias=False),
            #nn.BatchNorm2d(ndf * 8),
            #nn.LeakyReLU(0.2, inplace=True),
            # state size. (ndf*8) x 4 x 4
...
fixed_u = torch.randint(0, 2, (64, nu*nc,image_size,image_size), device=device)
...
with open('logbook/'+identity+'.csv', 'w') as csvfile:
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            u = torch.randint(0, 2, (b_size, nu*nc, image_size, image_size), dtype=torch.float, device=device)
    ...
    for snr in noise_ratio:
        ...
        for epoch in range(num_test_epochs):
            with torch.no_grad():
                ...
                u = torch.randint(0, 2, (batch_size, nu*nc, image_size, image_size), dtype=torch.float, device=device)
                fake = netG(noise,u)
                channel_noise = noise_std * torch.randn(fake.shape, dtype=torch.float, device=device)
